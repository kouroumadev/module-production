            ...
            <div class="col-md-12 col-sm-12">
                ...
                <nav aria-label="breadcrumb" role="navigation">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">AT</li>
                    </ol>
                </nav>
            </div>
        </div>
                    ...
                    @foreach ($trans as $key => $tran)
                        ...
                        @if ($tran != null)
                            ...
                            @else
                                <td>{{ \Carbon\Carbon::parse($tran->created_at)->format('d/m/Y') }}</td>
                            @endif --}}
                                <td><span class="badge badge-warning">{{ $tran->from->name }} ->
                                        {{ $tran->to->name }}</span></td>
                                @if ($current_date->diffInDays($tran->created_at) <= (int) $dead_name)
                                    <td>
                                        {{ $current_date->diffInDays($tran->created_at) }} <span class="badge "
                                            style=" text-align:center; background-color:green"> à temps</span>
                                    </td>
                                @else
                                    <td>
                                        {{ $current_date->diffInDays($tran->created_at) }} <span class="badge "
                                ...
                                @if ($tran->to->name == Auth::user()->dept->name)
                                    <td>
                                        <a class="btn btn-success"
                                            href="{{ route('pension.details', $tran->doc->id) }}">Traitement <i
