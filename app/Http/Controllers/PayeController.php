...
class PayeController extends Controller
{
    ...
    public function test()
    {
        ...
        foreach ($data as $user) {
            $user->example =
                '<a href="/user/' .
                $user->id .
                '"> Traitement <i class="fa fa-chevron-circle-right" aria-hidden="true"></i></a>';
    ...
    public function retraiteIndex(int $id)
    {
        ...
        $assignations = DB::table('pay_assignation')
            ->distinct()
            ->get(['assignation']);
        ...
        return view(
            'paye.retraite.index',
            compact('assignations', 'echeance', 'data')
        );
    }
    public function retraiteEdit(int $id)
    {
        ...
        if (
            Auth::user()->dept->name == 'PRODUCTION' ||
            Auth::user()->dept->name == 'PAYE'
        ) {
            $assignations = DB::table('pay_assignation')
                ->distinct()
                ->get(['assignation']);
            ...
            return view(
                'paye.retraite.edit',
                compact('assignations', 'echeance', 'retraite')
            );
        } else {
    ...
    public function retraiteSuspension(int $id)
    {
        ...
        return redirect(route('paye.suspension'))->with(
            'yes',
    ...
    public function etatPayementPdf()
    {
        ...
        $data = [
            ...
            'tot_montant_trim_reval' => AppHelper::getMoneyFormat(
                $tot_montant_trim_reval
            ),
            'tot_montant_mens_reval' => AppHelper::getMoneyFormat(
                $tot_montant_mens_reval
            ),
            'tot_montant_arriere' => AppHelper::getMoneyFormat(
                $tot_montant_arriere
            ),
            'tot_montant_a_payer' => AppHelper::getMoneyFormat(
                $tot_montant_a_payer
            ),
            'tot_montant_avance' => AppHelper::getMoneyFormat(
                $tot_montant_avance
            ),
    ...
    public function retraiteFilter(Request $request)
    {
        ...
        foreach ($data as $d) {
            ...
            $d->montant_a_payer =
                ...
                AppHelper::getMoneyFormat($d->montant_a_payer) .
                ' GNF</span>';
            $d->montant_arriere =
                '<span class="text-nowrap">' .
                AppHelper::getMoneyFormat($d->montant_arriere) .
                ' GNF</span>';
            $d->montant_avance =
                '<span class="text-nowrap">' .
                AppHelper::getMoneyFormat($d->montant_avance) .
                ' GNF</span>';
            $d->montant_comp =
                '<span class="text-nowrap">' .
                AppHelper::getMoneyFormat($d->montant_comp) .
                ' GNF</span>';
            $d->montant_comp_plus =
                '<span class="text-nowrap">' .
                AppHelper::getMoneyFormat($d->montant_comp_plus) .
                ' GNF</span>';
            $d->montant_trim =
                '<span class="text-nowrap">' .
                AppHelper::getMoneyFormat($d->montant_trim) .
                ' GNF</span>';
            $d->montant_trim_reval =
                '<span class="text-nowrap">' .
                AppHelper::getMoneyFormat($d->montant_trim_reval) .
                ' GNF</span>';
            $d->montant_mens_reval =
                '<span class="text-nowrap">' .
                AppHelper::getMoneyFormat($d->montant_mens_reval) .
                ' GNF</span>';
        }
        //    dd($data);
    ...
    public function retraitePreview(Request $request)
    {
        ...
        $data = Excel::toArray(
            new EtatRetraiteImport($request->echeance_id),
            $file
        );
    ...
    public function retraiteStoreAll(Request $request)
    {
        ...
        foreach ($data as $d) {
            ...
            $retraite->montant_trim_reval =
                (int) $d['montant_trimest'] +
                AppHelper::getPercentage(trim($d['montant_trimest']), 40);
        ...
        return redirect(
            route('payeRetraite.index', $request->echeance_id)
        )->with('yes', 'Enregistrer avec succes');
    }
    public function retraiteUpdate(Request $request)
    {
        ...
        return redirect(route('payeRetraite.index', $echeance_id))->with(
            'yes',
            'Enregistrer avec succes'
        );
    }
    public function getAss(Request $request)
    {
        ...
        $data = DB::table('pay_assignation')
            ->where('assignation', $value)
            ->select('assignation1')
            ->get();
